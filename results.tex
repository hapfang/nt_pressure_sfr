...
The first problem investigates the flow exiting the axial neutron reflector channels and mixing within an upper channel.
...
Meanwhile, an integral domain case is also simulated as a reference for the NekNek results.
...
The inlet semi-circular channels have a nominal radius of 2.5 while the full domain height is 18.4.
Both the upper and inlet channels are extruded.
The extrusion of the upper domain helps avoid the re-circulation issue at the outlet boundary face.
Meanwhile, the elongated inlet allows a fully developed turbulent inflow to be established.
Several Reynolds numbers are considered from 10,000 up to 900,000.
Since the non-dimensional calculations are carried out here, the increase of Reynolds number can be readily achieved by decreasing the flow viscosity.
...
It indicates that a strong mixing process is taking place when the flow comes out from the reflector and into the upper plenum.
...
Again, all the calculations are done in a non-dimensional manner, and the slight decrease in maximum TKE in Figure~\ref{fig:reflector_tke} should not be confused as the decrease in the actual TKE for higher Reynolds numbers.
In all simulations, the $y^+$ value is monitored and ideally the closest layer of GLL points should be within $y_p^+ < 1$ for the best accuracy.
So far, this restriction is only strictly kept in the case with $Re < 20K$.
...
To quantify the pressure change as the flow moves out of the semi-circular channels into a wider upper plenum, the area-averaged flow quantities are first extracted at both the domain inlet and outlet.
...
The pressure change in the current domain has two important components, the wall friction effect and the Bernoulli effect due to the change of channel shape.
...
The mean flow slows down as it moves from the narrow passage of the semi-circular channels into a wider plenum. Note that the hydrostatic pressure is excluded here by setting the gravitational acceleration ($g$) to zero.
The pressure change due to Bernoulli effect can be estimated by 
...
Unlike the pressure change induced by the Bernoulli effect, the pressure loss owing to the friction shows a log-linear dependency on the Reynolds number up to $Re = 10^5$ (as shown in Figure~\ref{fig:pressure_loss}), and then the trend is flattened at $Re > 10^5$.
It indicates that the form loss is becoming the dominant mechanism of the friction loss when the Reynolds number passes a certain threshold. 
A more detailed summary of pressure change results is provided by Table~\ref{tab:pressure}.
...
% \resizebox{0.48\textwidth}{!}{
 \begin{tabular}{ccccc} \hline \hline
   ...
   10,000    & -2.84E-03      & 4.60E-01      & -4.63E-01  \\
   20,000    & 1.43E-02       & 4.60E-01      & -4.46E-01  \\
   50,000    & 3.04E-02       & 4.60E-01      & -4.30E-01  \\
   100,000   & 3.87E-02       & 4.60E-01      & -4.22E-01  \\
   300,000   & 3.68E-02       & 4.60E-01      & -4.24E-01  \\
   900,000   & 3.34E-02       & 4.60E-01      & -4.27E-01  \\
   \hline \hline
\end{tabular}
...
The second case study aims to test a novel simulation technique, NekNek, and demonstrate its usefulness in complex engineering geometries.
A two-step approach is employed here: (i) a 2D backward facing step (BFS) case is first simulated to verify the NekNek performance; and (ii) the application of NekNek in simulating the flow entering a fuel pin bundle geometry.
The BFS case is selected because of the relevant flow physics involved, such as the separation and shear flow.
...
It is already qualitatively confirmed in Figure~\ref{fig:bfs_domain} that the NekNek approach can reproduce the overall flow solutions as compared to those from the integral domain with regular Nek5000 approach.
...
Identical results are produced from the NekNek approach and regular Nek5000 approach, which is a strong indicator of NekNek consistency.
...
A prototypical fuel bundle proposed for the SFR consists of 217 pins, and a simplified 7-pin bundle is studied here considering the available computational resources.
The 7-pin bundle is deemed adequate for the testing purpose and offers a much shorter turnaround time.
As shown in Figure~\ref{fig:bundle_domain}, the geometry can be divided into two partitions: an upper subdomain contains the pin bundle and supporting structures, and a lower subdomain which is a vertical channel. Note that the discretization is noticeably different over the subdomains, which is why NekNek can greatly ease the meshing process. 
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{./figures/bundle_neknek_results.png}
\caption{The quasi-steady-state streamwise velocity from the integral domain (a) and NekNek subdomains (b). }
